          ...
          <div key={column.key} className="board-column">
            ...
            <div className="issue-list">
              {isLoading ? <div className="empty-state">Loading issues...</div> : groupedIssues[column.key].length > 0 ? groupedIssues[column.key].map((issue) => (
                <article key={issue.id} className="issue-card"><div className="issue-top-row"><span className="issue-tag">{column.title}</span><span className="issue-meta">#{issue.id.slice(0, 6)}</span></div><h3>{issue.name}</h3><p className="issue-description">{issue.description}</p></article>
